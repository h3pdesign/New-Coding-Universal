...
# Function to get tags from Grok API with enhanced retry logic
def get_tags_from_grok(content_or_title):
    ...
    time.sleep(1)  # Baseline delay between all calls
    for attempt in range(retries):
        try:
            ...
            if e.response.status_code == 429:
                delay = 30 * (attempt + 1)  # Longer delays: 30, 60, 90, 120, 150 sec
                print(
                    f"Rate limit hit for '{content_or_title}', retrying in {delay} seconds... (Attempt {attempt + 1}/{retries})"
                )
                time.sleep(delay)
                if attempt == retries - 1:
                    print(f"Max retries reached for '{content_or_title}'")
                    return []
            else:
...
def fetch_pocket_articles():
    all_articles = {}
    offset = 0
    count = 100  # Stable count to avoid 413
    retries = 3
        ...
        for attempt in range(retries):
            try:
                ...
                if hasattr(e, "response") and e.response:
                    ...
                    if status == 413:
                        print("Payload too large. Reducing count...")
...
def main():
    ...
    if articles is not None:
        ...
        article_tags = {}
        failed_articles = []  # Track articles needing retry
        for item_id, article in articles.items():
            ...
            if tags:
                ...
                failed_articles.append((item_id, article))

        if article_tags:
            tag_pocket_articles(article_tags)

        # Retry failed articles after a longer delay
        if failed_articles:
            print(
                f"Retrying {len(failed_articles)} failed articles after 5-minute delay..."
            )
            time.sleep(300)  # 5 minutes
            retry_tags = {}
            for item_id, article in failed_articles:
                title = article.get(
                    "resolved_title", article.get("given_title", "Untitled")
                )
                ...
                if tags:
                    print(f"Generated tags: {tags}")
                    retry_tags[item_id] = tags
            if retry_tags:
                tag_pocket_articles(retry_tags)
    else:
